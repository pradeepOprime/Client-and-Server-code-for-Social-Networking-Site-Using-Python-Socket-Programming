...
#Write_LOg("||Request|| ","Ecxrupted request","dummy")
...
def Write_LOg(string1,string2,string3):
	LogFiel = open("ServerLog.txt","a+")
	time_now = datetime.datetime.now()
	DT_string = str(time_now)
	Write_LINE = "Reequest Log server::||FROM|| %s || ::Type|| %s|| \
		request:: %s : at :: %s\n"%(string1,string2,string3,DT_string)
	LogFiel.write(Write_LINE)
	LogFiel.close()
...
def Recive_Decrypt():
	...
	Write_LOg("KNOWN_CLIENT","Ecxrupted Request",Plain_block)
...
def Encrypt_Send(E_Block):
	...
	Write_LOg("KNOWN_CLIENT","Eencrypted Response",E_BlockX16)
